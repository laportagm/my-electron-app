...
const enhancedFs = {
  ...
  readFileSync: (p: string, encoding: BufferEncoding = 'utf8', maxSize: number = 1024 * 1024): string | null => {
    try {
...
const logger = {
  ...
  error: (message: string, error?: Error, ...args: any[]): void => {
    enhancedIpc.send('log', 'error', message, error?.message, error?.stack, ...args);
...
const diagnostics = {
  ...
  checkConnectivity: async (): Promise<Record<string, boolean>> => {
    return {
 ...
 */
async function testIpcConnectivity(): Promise<boolean> {
  try {
